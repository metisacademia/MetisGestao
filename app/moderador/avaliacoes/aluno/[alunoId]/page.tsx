...
export default function ModeradorAvaliarAlunoPage() {
    ...
    async function carregarDados() {
      try {
        ...
        const avaliacaoRes = await apiCall(
          `/api/moderador/salvar-avaliacao?alunoId=${alunoId}&mes=${mes}&ano=${ano}`
        );
  ...
  const buscarProximoEAnteriorAluno = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/moderador/turmas/${turmaId}`);
  ...
  const handleSalvarENavegar = async (direcao: 'proximo' | 'anterior') => {
    ...
    try {
      const response = await apiCall(`/api/moderador/salvar-avaliacao`, {
        method: 'POST',
      ...
      if (direcao === 'proximo' && proximoAluno) {
        router.push(`/moderador/avaliacoes/aluno/${proximoAluno.id}?mes=${mes}&ano=${ano}&turmaId=${turmaId}`);
      } else if (direcao === 'anterior' && alunoAnterior) {
        router.push(`/moderador/avaliacoes/aluno/${alunoAnterior.id}?mes=${mes}&ano=${ano}&turmaId=${turmaId}`);
      } else {
  ...
  const handleReabrirAvaliacao = async () => {
    ...
    try {
      const response = await apiCall(`/api/moderador/reabrir-avaliacao`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
