...
export default function DetalhesAlunoPage() {
    ...
    async function carregarDados() {
      ...
      try {
        const [resAluno, resEventos, resPresencas] = await Promise.all([
          ...
          fetch(`/api/coordenador/alunos/${alunoId}/eventos`),
          fetch(`/api/coordenador/alunos/${alunoId}/presencas`),
        ]);
  ...
  async function criarUsuario() {
    ...
    setErroUsuario(null);
    try {
      const res = await fetch(`/api/coordenador/alunos/${alunoId}/criar-usuario`, {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        const resAluno = await fetch(`/api/coordenador/alunos/${alunoId}`);
        if (resAluno.ok) {
          setAluno(await resAluno.json());
        }
      } else {
  ...
  async function salvarEvento() {
    ...
    try {
      const res = await fetch(`/api/coordenador/alunos/${alunoId}/eventos`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function salvarPresenca() {
    ...
    try {
      const res = await fetch(`/api/coordenador/alunos/${alunoId}/presencas`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-4">
        <Link href={`/coordenador/relatorios?aluno=${alunoId}`}>
