...
export default function RelatorioAnualPage() {
  ...
  useEffect(() => {
    async function carregarDados() {
      ...
      setLoading(true);
      try {
        const res = await fetch(`/api/coordenador/relatorios/aluno/${alunoId}/anual?ano=${anoSelecionado}`);
        if (res.ok) {
          const data = await res.json();
          setDados(data);
        } else {
          router.push('/coordenador/relatorios');
        }
      } catch (error) {
  ...
  const handleExportarCSV = () => {
    if (alunoId) {
      window.open(`/api/coordenador/relatorios/exportar/aluno/${alunoId}`, '_blank');
    }
  };
